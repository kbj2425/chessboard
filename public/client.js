(function () {
  ...
  socket.on('auth:ready', ({ username, stats, ongoingRooms }) => {
    ...
    socket.emit('lobby:list', (res) => renderRoomList(res.rooms));
    renderReconnectBanner(ongoingRooms);
  });
  ...
  function renderReconnectBanner(rooms) {
    const banner = $('#reconnectBanner');
    if (!rooms || !rooms.length) { banner.hidden = true; return; }
    banner.hidden = false;
    const r = rooms[0];
    banner.innerHTML = `<span>진행 중인 대국이 있습니다 — ${MODE_LABEL[r.mode]}</span>`;
    const btn = document.createElement('button');
    btn.className = 'primary-btn';
    btn.textContent = '재접속하기';
